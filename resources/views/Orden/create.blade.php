...
@section('contenido')
<h1>ORDENES DE SERVICIO MASIVA</h1>
...
<form method="POST" action="{{ url('operacion/orden_servicio') }}">
  	...
  	<div class="row">
        ...
        <input type="text" name="num_actual" id="num_actual" value="{{ $c->num_actual }}" hidden>
            <input type="text" name="cod_orden"  class="form-control bg-info text-white" value="{{ $c->prefijo_doc.' - '.$c->num_actual }}" readonly="readonly" required>
      @endforeach
    	</div>
